...
void Clock20ms_Start(void) 
{
    ...
    Clock20ms_CLKEN |= Clock20ms_CLKEN_MASK;
	Clock20ms_CLKSTBY |= Clock20ms_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock20ms_Stop
********************************************************************************
*
* Summary:
...
void Clock20ms_Stop(void) 
{
    ...
    Clock20ms_CLKEN &= (uint8)(~Clock20ms_CLKEN_MASK);
	Clock20ms_CLKSTBY &= (uint8)(~Clock20ms_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock20ms_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock20ms_CLKEN & Clock20ms_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock20ms_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock20ms_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock20ms__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock20ms_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock20ms_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock20ms_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock20ms_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock20ms_GetSourceRegister();
    uint16 oldDivider = Clock20ms_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock20ms_CLKEN & Clock20ms_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock20ms_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock20ms_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock20ms_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock20ms_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock20ms_MOD_SRC |= modeBitMask & (uint8)Clock20ms_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock20ms_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock20ms_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock20ms_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock20ms_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock20ms_MOD_SRC & (uint8)(Clock20ms_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock20ms_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock20ms_GetDividerRegister();
    uint8 oldSrc = Clock20ms_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock20ms_MOD_SRC |= CYCLK_SSS;
        Clock20ms_MOD_SRC =
            (Clock20ms_MOD_SRC & (uint8)(~Clock20ms_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock20ms_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock20ms_MOD_SRC & Clock20ms_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock20ms_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock20ms_PHASE = clkPhase & Clock20ms_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock20ms_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock20ms_PHASE & Clock20ms_PHASE_MASK;
}

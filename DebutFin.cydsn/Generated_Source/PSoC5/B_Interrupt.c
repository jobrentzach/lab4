...
void B_Interrupt_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)B_Interrupt__INTC_NUMBER] = address;
}
...
cyisraddress B_Interrupt_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)B_Interrupt__INTC_NUMBER];
}
...
{
    *B_Interrupt_INTC_PRIOR = priority << 5;
}
...
uint8 B_Interrupt_GetPriority(void)
{
    ...
    priority = *B_Interrupt_INTC_PRIOR >> 5;

...
void C_SetDriveMode(uint8 mode)
{
	...
	CyPins_SetPinDriveMode(C_1, mode);
	CyPins_SetPinDriveMode(C_2, mode);
	CyPins_SetPinDriveMode(C_3, mode);
}
...
/* If interrupt is connected for this Pins component */ 
    ...
    *******************************************************************************/
    void C_SetInterruptMode(uint16 position, uint16 mode)
    {
		...
		if((position & C_1_INTR) != 0u) 
		{ 
			 C_1_INTTYPE_REG = (uint8)mode; 
		} 
		if((position & C_2_INTR) != 0u) 
		{ 
			 C_2_INTTYPE_REG = (uint8)mode; 
		} 
		if((position & C_3_INTR) != 0u) 
		{ 
			 C_3_INTTYPE_REG = (uint8)mode; 
		}
    }
